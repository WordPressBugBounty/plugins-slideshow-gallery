		...
		<tr>
			...
			<td>
				<label class="slideshow-custom-checkbox">
					<input <?php echo (!empty($language_external)) ? 'checked="checked"' : ''; ?> type="checkbox" name="language_external" value="1" id="language_external" />
					<span class="check-icon"></span>
					<?php _e('Yes, load external language file', 'slideshow-gallery'); ?>
				</label>
...
<?php if (current_user_can('edit_users') || is_super_admin()) : ?>
	<div class="slideshow-permission-list-wrapper">
		<table class="form-table">
			<thead>
				<tr>
					<th>&nbsp;</th>
						...
						<th style="white-space:nowrap; font-weight:bold; text-align:center;">
							<label class="slideshow-custom-checkbox">
								<?php if ($role_key != "administrator") : ?>
									<input type="checkbox" name="sectionsrolescheckall<?php echo esc_attr($role_key); ?>" value="1" id="sectionsrolescheckall<?php echo esc_attr($role_key); ?>" onclick="jqCheckAll(this, false, 'permissions[<?php echo esc_attr($role_key); ?>]');" />
									<span class="check-icon"></span>
								<?php endif; ?>
